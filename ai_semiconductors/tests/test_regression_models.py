...
def test_MLR():
    ...
    assert type(rmse) == np.float64, "output type for linear regression is wrong"
...
def test_LassoReg():
    ...
    import numpy as np
    X = np.array([[1, 1], [1, 2], [2, 2], [2, 3], [1, 1], [1, 2], [2, 2], [2, 3]])
    y = np.dot(X, np.array([1, 2])) + 3
    model = LassoReg(X, y)
...
def test_krr():
    ...
    assert type(result) == pd.DataFrame, "Output type of Kernel Ridge Regression model is wrong"
